...
# Save permittivity tensor (real + imag) to file
filename = "permittivity_kgd_$(kgrid_density).dat"
open(filename, "w") do f
    # Metadata headers (consistent with shift current format)
    ...
    write(f, "# Columns: ω, (ε_{αβ}^Re, ε_{αβ}^Im) for α,β=1..3\n\n")

    # Data rows
    for i in 1:num_ωs
        row = [ωs[i]]  # frequency (omega)
        for α in 1:3, β in 1:3
            push!(row, tensor[α, β, i, 1])  # Real part
            push!(row, tensor[α, β, i, 2])  # Imag part
        end
        writedlm(f, [row], " ")
    end
end

println("Permittivity tensor (real+imag) written to $filename")
