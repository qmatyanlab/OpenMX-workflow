...
class BatchRunner:
    ...
    def calc(self, batch_id, kppa=2e3):
        ...
        for i in range((batch_id-1)*10, batch_id*10):
            ...
            wf = set_execution_options(wf, category=">>this need to match with my_fworker.yaml<<")
    ...
    def run_batch(self):
        ...
        with Pool(20) as p:
            p.map(self.calc, range(1, int(self.total_sts/4)+1))
        # clean up the memory
        p.close()
        p.join()
...
if __name__ == "__main__":
    runner = BatchRunner(
        "/path/to/your/config/my_launchpad.yaml", 
        [
            '/path/to/your/mp_structure_0-500.json',
            '/path/to/your/mp_structure_500-1000.json',
            '/path/to/your/mp_structure_1000-1500.json',
            '/path/to/your/mp_structure_1500-2000.json',
        ]
    )
